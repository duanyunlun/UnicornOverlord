...
internal class ViewModel : INotifyPropertyChanged
{
	...
	internal static ObservableCollection<ModModule> CreateModModules()
	{
		...
		[
			...
			new() { Key = "fort_editor", Category = "据点", Name = "据点雇佣编辑器", Description = "按 63 个具体据点选择全部 248 个招募位置并修改可招募职业；选择后会载入原版职业，手动选择不受转职阶段限制。", IsAvailable = true, CalibrationState = "63 个据点 / 248 项已校准", RecordId = 1, Warning = "仅写职业字段，亚洲版记录中的性别与附加类型保持不变。" },
			new() { Key = "mine_editor", Category = "采矿", Name = "采矿掉落编辑器", Description = "按五个地区采掘场选择 63 条具体原版掉落，修改物品、相对权重、挖掘目标和单局上限。", IsAvailable = true, CalibrationState = "5 个采掘场 / 63 项已校准", Mine = new MineEditorState(), Warning = "藏宝图等一次性物品由游戏另行限制；提高权重时也要检查单局上限。" },
